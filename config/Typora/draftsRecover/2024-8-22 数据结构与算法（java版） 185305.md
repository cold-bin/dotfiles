...
- 快排优化：**选择的基准在我们的理想状况下应该是尽量将数组分区成左右两部分长度相差不大的序列才对。**避免分区的时候出现极端的分区（左边或右边只有一个元素有序，也就是一趟排序只能确定一个元素的位置），那么快排会退化为O2。
  ...
  - 三数取中法。即**将数组的第一个，中间，最后一个元素排序后的中位数作为基准进行分区划分**。比随机更靠谱，中位数应该更接近中间值，这样可以防止分区排序一边倒的情况

  - 三路切分（三路快排）。

    在实际的应用中，经常会**出现大量重复元素**的数组，对于这些重复的元素，其实是不需要再继续排序了，但是快速排序**依然会将它们切分成更小的数组**，这使得快速排序的有很大的改进空间。

    一个简单的想法就是**将数组切分成三部分，分别对应小于、等于、大于基准的数组元素**，具体的步骤如下图所示：

    **1、维护一个指针lt使得[lo,lt-1]中的元素都小于基准，一个指针gt使得[gt+1,hi]中的元素都大于基准，一个指针i使得[lt,i-1]中的元素都等于基准。**

    ![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6722d14dc83e47398cf542e012f85ad1~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)

    2、接下来**将与基准相等的值都移动到中间来**，按照入如下算法:

    ![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9be6ef7f49c0470090c2305c81827b8f~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)

    3、一顿猛如虎的操作下来，就变成了：

    ![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/16272f0704394d51b34404105cfb77cf~tplv-k3u1fbpfcp-zoom-in-crop-mark:1512:0:0:0.awebp?)

    4、最后再对小于以及大于基准的两部分子数组进行同样步骤即可

    作者：打代码的胖虎
    链接：https://juejin.cn/post/7057181977002115103
    来源：稀土掘金
